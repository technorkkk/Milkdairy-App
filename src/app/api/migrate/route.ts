...
// Temporary migration endpoint - creates all tables and RLS policies
// This should be removed after the database is set up
export async function POST() {
  const DATABASE_URL = process.env.DATABASE_URL || 'postgresql://postgres:Rahul1234%40.com@db.qrgsabeyzkcxkfeuhfuy.supabase.co:5432/postgres?sslmode=require';
  ...
  try {
    // Dynamic import of pg since it's only needed for migration
...
DROP POLICY IF EXISTS "Allow anon select on User" ON "User";
DROP POLICY IF EXISTS "Allow anon insert on User" ON "User";
DROP POLICY IF EXISTS "Allow anon update on User" ON "User";
DROP POLICY IF EXISTS "Allow anon delete on User" ON "User";
DROP POLICY IF EXISTS "Allow authenticated select on User" ON "User";
DROP POLICY IF EXISTS "Allow authenticated insert on User" ON "User";
DROP POLICY IF EXISTS "Allow authenticated update on User" ON "User";
DROP POLICY IF EXISTS "Allow authenticated delete on User" ON "User";

DROP POLICY IF EXISTS "Allow anon select on Dairy" ON "Dairy";
DROP POLICY IF EXISTS "Allow anon insert on Dairy" ON "Dairy";
DROP POLICY IF EXISTS "Allow anon update on Dairy" ON "Dairy";
DROP POLICY IF EXISTS "Allow anon delete on Dairy" ON "Dairy";
DROP POLICY IF EXISTS "Allow authenticated select on Dairy" ON "Dairy";
DROP POLICY IF EXISTS "Allow authenticated insert on Dairy" ON "Dairy";
DROP POLICY IF EXISTS "Allow authenticated update on Dairy" ON "Dairy";
DROP POLICY IF EXISTS "Allow authenticated delete on Dairy" ON "Dairy";

DROP POLICY IF EXISTS "Allow anon select on Customer" ON "Customer";
DROP POLICY IF EXISTS "Allow anon insert on Customer" ON "Customer";
DROP POLICY IF EXISTS "Allow anon update on Customer" ON "Customer";
DROP POLICY IF EXISTS "Allow anon delete on Customer" ON "Customer";
DROP POLICY IF EXISTS "Allow authenticated select on Customer" ON "Customer";
DROP POLICY IF EXISTS "Allow authenticated insert on Customer" ON "Customer";
DROP POLICY IF EXISTS "Allow authenticated update on Customer" ON "Customer";
DROP POLICY IF EXISTS "Allow authenticated delete on Customer" ON "Customer";

DROP POLICY IF EXISTS "Allow anon select on MilkRate" ON "MilkRate";
DROP POLICY IF EXISTS "Allow anon insert on MilkRate" ON "MilkRate";
DROP POLICY IF EXISTS "Allow anon update on MilkRate" ON "MilkRate";
DROP POLICY IF EXISTS "Allow anon delete on MilkRate" ON "MilkRate";
DROP POLICY IF EXISTS "Allow authenticated select on MilkRate" ON "MilkRate";
DROP POLICY IF EXISTS "Allow authenticated insert on MilkRate" ON "MilkRate";
DROP POLICY IF EXISTS "Allow authenticated update on MilkRate" ON "MilkRate";
DROP POLICY IF EXISTS "Allow authenticated delete on MilkRate" ON "MilkRate";

DROP POLICY IF EXISTS "Allow anon select on Delivery" ON "Delivery";
DROP POLICY IF EXISTS "Allow anon insert on Delivery" ON "Delivery";
DROP POLICY IF EXISTS "Allow anon update on Delivery" ON "Delivery";
DROP POLICY IF EXISTS "Allow anon delete on Delivery" ON "Delivery";
DROP POLICY IF EXISTS "Allow authenticated select on Delivery" ON "Delivery";
DROP POLICY IF EXISTS "Allow authenticated insert on Delivery" ON "Delivery";
DROP POLICY IF EXISTS "Allow authenticated update on Delivery" ON "Delivery";
DROP POLICY IF EXISTS "Allow authenticated delete on Delivery" ON "Delivery";

DROP POLICY IF EXISTS "Allow anon select on Payment" ON "Payment";
DROP POLICY IF EXISTS "Allow anon insert on Payment" ON "Payment";
DROP POLICY IF EXISTS "Allow anon update on Payment" ON "Payment";
DROP POLICY IF EXISTS "Allow anon delete on Payment" ON "Payment";
DROP POLICY IF EXISTS "Allow authenticated select on Payment" ON "Payment";
DROP POLICY IF EXISTS "Allow authenticated insert on Payment" ON "Payment";
DROP POLICY IF EXISTS "Allow authenticated update on Payment" ON "Payment";
DROP POLICY IF EXISTS "Allow authenticated delete on Payment" ON "Payment";

DROP POLICY IF EXISTS "Allow anon select on Expense" ON "Expense";
DROP POLICY IF EXISTS "Allow anon insert on Expense" ON "Expense";
DROP POLICY IF EXISTS "Allow anon update on Expense" ON "Expense";
DROP POLICY IF EXISTS "Allow anon delete on Expense" ON "Expense";
DROP POLICY IF EXISTS "Allow authenticated select on Expense" ON "Expense";
DROP POLICY IF EXISTS "Allow authenticated insert on Expense" ON "Expense";
DROP POLICY IF EXISTS "Allow authenticated update on Expense" ON "Expense";
DROP POLICY IF EXISTS "Allow authenticated delete on Expense" ON "Expense";

DROP POLICY IF EXISTS "Allow anon select on InventoryItem" ON "InventoryItem";
DROP POLICY IF EXISTS "Allow anon insert on InventoryItem" ON "InventoryItem";
DROP POLICY IF EXISTS "Allow anon update on InventoryItem" ON "InventoryItem";
DROP POLICY IF EXISTS "Allow anon delete on InventoryItem" ON "InventoryItem";
DROP POLICY IF EXISTS "Allow authenticated select on InventoryItem" ON "InventoryItem";
DROP POLICY IF EXISTS "Allow authenticated insert on InventoryItem" ON "InventoryItem";
DROP POLICY IF EXISTS "Allow authenticated update on InventoryItem" ON "InventoryItem";
DROP POLICY IF EXISTS "Allow authenticated delete on InventoryItem" ON "InventoryItem";

DROP POLICY IF EXISTS "Allow anon select on LedgerEvent" ON "LedgerEvent";
DROP POLICY IF EXISTS "Allow anon insert on LedgerEvent" ON "LedgerEvent";
DROP POLICY IF EXISTS "Allow anon update on LedgerEvent" ON "LedgerEvent";
DROP POLICY IF EXISTS "Allow anon delete on LedgerEvent" ON "LedgerEvent";
DROP POLICY IF EXISTS "Allow authenticated select on LedgerEvent" ON "LedgerEvent";
DROP POLICY IF EXISTS "Allow authenticated insert on LedgerEvent" ON "LedgerEvent";
DROP POLICY IF EXISTS "Allow authenticated update on LedgerEvent" ON "LedgerEvent";
DROP POLICY IF EXISTS "Allow authenticated delete on LedgerEvent" ON "LedgerEvent";

DROP POLICY IF EXISTS "Allow anon select on QuantityHistory" ON "QuantityHistory";
DROP POLICY IF EXISTS "Allow anon insert on QuantityHistory" ON "QuantityHistory";
DROP POLICY IF EXISTS "Allow anon update on QuantityHistory" ON "QuantityHistory";
DROP POLICY IF EXISTS "Allow anon delete on QuantityHistory" ON "QuantityHistory";
DROP POLICY IF EXISTS "Allow authenticated select on QuantityHistory" ON "QuantityHistory";
DROP POLICY IF EXISTS "Allow authenticated insert on QuantityHistory" ON "QuantityHistory";
DROP POLICY IF EXISTS "Allow authenticated update on QuantityHistory" ON "QuantityHistory";
DROP POLICY IF EXISTS "Allow authenticated delete on QuantityHistory" ON "QuantityHistory";

DROP POLICY IF EXISTS "Allow anon select on AuditLog" ON "AuditLog";
DROP POLICY IF EXISTS "Allow anon insert on AuditLog" ON "AuditLog";
DROP POLICY IF EXISTS "Allow anon update on AuditLog" ON "AuditLog";
DROP POLICY IF EXISTS "Allow anon delete on AuditLog" ON "AuditLog";
DROP POLICY IF EXISTS "Allow authenticated select on AuditLog" ON "AuditLog";
DROP POLICY IF EXISTS "Allow authenticated insert on AuditLog" ON "AuditLog";
DROP POLICY IF EXISTS "Allow authenticated update on AuditLog" ON "AuditLog";
DROP POLICY IF EXISTS "Allow authenticated delete on AuditLog" ON "AuditLog";

DROP POLICY IF EXISTS "Allow anon select on SyncQueue" ON "SyncQueue";
DROP POLICY IF EXISTS "Allow anon insert on SyncQueue" ON "SyncQueue";
DROP POLICY IF EXISTS "Allow anon update on SyncQueue" ON "SyncQueue";
DROP POLICY IF EXISTS "Allow anon delete on SyncQueue" ON "SyncQueue";
DROP POLICY IF EXISTS "Allow authenticated select on SyncQueue" ON "SyncQueue";
DROP POLICY IF EXISTS "Allow authenticated insert on SyncQueue" ON "SyncQueue";
DROP POLICY IF EXISTS "Allow authenticated update on SyncQueue" ON "SyncQueue";
DROP POLICY IF EXISTS "Allow authenticated delete on SyncQueue" ON "SyncQueue";

-- Create RLS policies for all tables (anon + authenticated, all operations)
...
`;
    ...
    try {
      ...
      return NextResponse.json({
        success: true,
        message: 'All tables and RLS policies created successfully!',
      });
    } finally {
      client.release();
      await pool.end();
